...
export default async function updateStudyListhandler(request: NextApiRequest, response: NextApiResponse) {
  try {
    ...
    const result = await db.collection('study').findOne({ _id: new ObjectId(studyDataId) })
